  ...
  async function load() {
    try {
      ...
      const [s, risk, evs, arts] = await Promise.all([
        ...
        getJson("/api/stats/top-risky-province?hours=24"),
        getJson(
          `/api/events?limit=50${
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-6">
        ...
        <StatCard
          ...
          sub={`${riskiest?.events_24h ?? 0} sự kiện trong 24h`}
        />
      </div>
            ...
            <div className="mt-3 flex items-center justify-between px-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm text-gray-700 px-2">
                  Trang {page} / {Math.max(1, Math.ceil(articles.length / 10))}
                </div>
                <button
                  className="px-3 py-1 rounded-xl border bg-white text-sm text-gray-700 disabled:opacity-50"
                  onClick={() =>
                    setPage((p) => Math.min(Math.ceil(articles.length / 10) || 1, p + 1))
                  }
