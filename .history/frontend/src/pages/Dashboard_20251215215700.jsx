...
// VietnamMap removed from Dashboard per request (map hidden)
import {
  LineChart,
  Line,
  BarChart,
...
export default function Dashboard() {
  ...
  async function load() {
    try {
      ...
      // Filter out articles whose disaster_type is 'unknown' to avoid showing "Khác"
      setArticles(
        arts.filter((a) => a.disaster_type && a.disaster_type !== "unknown")
      );
    } catch (e) {
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-6">
        <StatCard
          ...
          sub="Tổng bài báo thông tin thu thập được"
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
        <div className="rounded-2xl border border-gray-300 bg-white p-4">
          <div className="text-sm font-semibold text-gray-900">Bản đồ (đã ẩn)</div>
          <div className="text-xs text-gray-600 mt-1">
            Bản đồ đã được tạm ẩn theo yêu cầu. Liên hệ để bật lại hoặc thay đổi provider.
          </div>
        </div>
        <div className="rounded-2xl border border-gray-300 bg-white p-4">
