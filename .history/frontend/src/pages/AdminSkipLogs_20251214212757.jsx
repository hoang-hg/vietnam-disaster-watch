...
export default function AdminSkipLogs() {
  ...
  async function label(item, lab) {
    try {
      await fetch(API_BASE + "/api/admin/label", {
        method: "POST",
  ...
  return (
    <div className="p-4">
      ...
      {!loading && items.length === 0 && !error && (
        <div className="text-sm text-gray-500">Không có bản ghi nào. Hãy chắc backend đang chạy và crawler đã ghi `logs/skip_debug.jsonl`.</div>
      )}
        ...
        {items.map((it, idx) => (
          <div key={idx} className="p-3 border rounded bg-white">
            <div className="text-sm text-gray-600">
              {it.timestamp} — <span className="font-medium">{it.source}</span> — <span className="italic">{it.action}</span>
            </div>
            <div className="font-semibold mt-1">{it.title}</div>
            <div className="text-xs text-gray-700 break-words">{it.url}</div>
            {it.diagnose && <div className="text-xs text-gray-500 mt-1">Diag: {it.diagnose.reason} (score {it.diagnose.score})</div>}
            <div className="mt-2">
              <a className="mr-3 text-sm text-blue-600" href={it.url} target="_blank" rel="noreferrer">Open article</a>
              <button onClick={() => label(it, "accept")} className="mr-2 px-2 py-1 bg-green-600 text-white text-sm">Accept</button>
              <button onClick={() => label(it, "reject")} className="px-2 py-1 bg-red-600 text-white text-sm">Reject</button>
            </div>
          </div>
        ))}
      </div>
