...
export default function Dashboard() {
  ...
  async function load() {
    try {
      setError(null);
      const [s, evs, arts, tl, hm] = await Promise.all([
        ...
        getJson(
          `/api/articles/latest?limit=50${
            type ? `&type=${encodeURIComponent(type)}` : ""
          }${province ? `&province=${encodeURIComponent(province)}` : ""}`
        ),
        getJson("/api/stats/timeline?hours=24"),
        getJson("/api/stats/heatmap?hours=24"),
      ]);
      setStats(s);
      setEvents(evs);
      setArticles(arts);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
        <div className="rounded-2xl border border-gray-300 bg-white p-4">
          <div className="text-sm font-semibold text-gray-900">
            Timeline sự kiện (24h)
          </div>
          <div className="text-xs text-gray-600 mt-1">
            Xu hướng số sự kiện theo giờ
          </div>
          {timeline.length > 0 ? (
            ...
            <div className="mt-4 text-sm text-gray-500">Chưa có dữ liệu</div>
          )}
        </div>

        <div className="rounded-2xl border border-gray-300 bg-white p-4">
          <div className="text-sm font-semibold text-gray-900">
            Top tỉnh có sự kiện (24h)
          </div>
          <div className="text-xs text-gray-600 mt-1">
            Các tỉnh có nhiều sự kiện nhất
          </div>
          {heatmap.length > 0 ? (
            <ResponsiveContainer width="100%" height={300} className="mt-4">
              <BarChart data={heatmap.slice(0, 10)}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                <XAxis dataKey="province" tick={{ fontSize: 11 }} angle={-45} textAnchor="end" height={80} />
                <YAxis tick={{ fontSize: 12 }} />
                <Tooltip contentStyle={{ backgroundColor: "#fff", border: "1px solid #e5e7eb" }} />
