      ...
      {ev.articles && ev.articles.length > 0 ? (
        (() => {
          const combined = ev.articles
            .map((a) => (a.full_text && a.full_text.length ? a.full_text : a.summary || ""))
            .filter(Boolean)
            .slice(0, 3)
            .join("\n\n");
          if (!combined) return null;
          const limit = 800;
          const short = combined.length > limit && !expandedSummary ? combined.slice(0, limit).trim() + "…" : combined;
          return (
            <div className="mt-4 text-sm text-gray-700 bg-white p-4 rounded border border-gray-200">
              ...
              <div className="text-sm whitespace-pre-line text-gray-700">{short}</div>
