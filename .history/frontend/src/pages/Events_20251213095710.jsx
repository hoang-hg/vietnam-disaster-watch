  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="flex items-end justify-between gap-4">
        <div>
          <div className="text-2xl font-semibold text-gray-900">Danh sách sự kiện</div>
          <div className="text-sm text-gray-600 mt-1">Nhóm theo loại + tỉnh + ngày • Ưu tiên sự kiện đa nguồn</div>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        {events.map(e => (
          <a key={e.id} href={`/events/${e.id}`} className="block rounded-2xl border border-gray-300 bg-white hover:bg-gray-50 p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="font-semibold leading-snug text-gray-900">{e.title}</div>
            ...
            <div className="mt-3 grid grid-cols-4 gap-2 text-xs">
              <div className="rounded-xl border border-gray-300 bg-gray-50 p-2">
                <div className="text-gray-600 font-medium">Chết</div><div className="font-semibold text-gray-900">{e.deaths ?? '—'}</div>
              </div>
              <div className="rounded-xl border border-gray-300 bg-gray-50 p-2">
                <div className="text-gray-600 font-medium">Mất tích</div><div className="font-semibold text-gray-900">{e.missing ?? '—'}</div>
              </div>
              <div className="rounded-xl border border-gray-300 bg-gray-50 p-2">
                <div className="text-gray-600 font-medium">Bị thương</div><div className="font-semibold text-gray-900">{e.injured ?? '—'}</div>
              </div>
              <div className="rounded-xl border border-gray-300 bg-gray-50 p-2">
                <div className="text-gray-600 font-medium">Thiệt hại</div><div className="font-semibold text-gray-900">{fmtVndBillion(e.damage_billion_vnd)}</div>
              </div>
            </div>
          </a>
        ))}
