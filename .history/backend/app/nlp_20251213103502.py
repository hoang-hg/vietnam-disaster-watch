...
RE_DEATHS = re.compile(r"(?:chết|tử|tử\s*vong|mất\s*mạng).*?(\d[\d\.\,]*)\s*người|(\d[\d\.\,]*)\s*người\s*(?:chết|tử|tử\s*vong|mất\s*mạng)", re.IGNORECASE)
RE_MISSING = re.compile(r"(?:mất\s*tích|biến\s*mất|không\s*tìm).*?(\d[\d\.\,]*)\s*người|(\d[\d\.\,]*)\s*người\s*(?:mất\s*tích|biến\s*mất)", re.IGNORECASE)
RE_INJURED = re.compile(r"(?:bị\s*thương|thương\s*tích|bị\s*tổn\s*thương).*?(\d[\d\.\,]*)\s*người|(\d[\d\.\,]*)\s*người\s*(?:bị\s*thương|thương\s*tích)", re.IGNORECASE)
RE_DAMAGE = re.compile(r"(?:thiệt\s*hại|tổn\s*thất|mất\s*mát).*?(\d[\d\.\,]*)\s*(?:tỉ|tỷ)\s*đồng|(\d[\d\.\,]*)\s*(?:tỉ|tỷ)\s*đồng", re.IGNORECASE)
...
PROVINCES = [
    "Hà Nội","TP.HCM","Thành phố Hồ Chí Minh","Đà Nẵng","Huế","Nghệ An","Hà Tĩnh","Quảng Bình","Quảng Trị",
    "Thừa Thiên Huế","Quảng Nam","Quảng Ngãi","Bình Định","Phú Yên","Khánh Hòa","Ninh Thuận","Bình Thuận",
    "Gia Lai","Kon Tum","Đắk Lắk","Đắk Nông","Lâm Đồng","Lào Cai","Yên Bái","Sơn La","Điện Biên","Lai Châu",
    "Thanh Hóa","Quảng Ninh","Hải Phòng","Hải Dương","Bắc Giang","Bắc Ninh","Thái Nguyên","Cao Bằng","Lạng Sơn",
    "Bình Dương","Đồng Nai","Bà Rịa - Vũng Tàu","Long An","Tiền Giang","Bến Tre","Trà Vinh","Sóc Trăng","Cà Mau",
]
...
def extract_impacts(text: str) -> dict:
    ...
    if m: deaths = _to_int(m.group(1))
    m = RE_MISSING.search(text)
    if m: missing = _to_int(m.group(1))
    m = RE_INJURED.search(text)
    if m: injured = _to_int(m.group(1))

    damages = [(_to_float(x), unit) for x, unit in RE_DAMAGE.findall(text)]
    if damages:
        damage = max(d[0] for d in damages)
