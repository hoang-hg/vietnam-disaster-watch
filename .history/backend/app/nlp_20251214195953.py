...
def contains_disaster_keywords(text: str) -> bool:
    ...
    if RE_AGENCY.search(t):
        return True

    # 4) Province/region mention — indicates location-specific reporting
    try:
        if extract_province(t) != "unknown":
            return True
    except Exception:
        pass
    ...
    if found:
        # if any non-ambiguous keyword found, accept
        if any(k not in ambiguous for k in found):
            return True
        # ambiguous-only matches are not sufficient
