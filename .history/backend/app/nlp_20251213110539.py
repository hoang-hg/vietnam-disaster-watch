...
def _build_impact_patterns():
    ...
    for impact_type, keywords in IMPACT_KEYWORDS.items():
        ...
        if impact_type == "deaths":
            patterns["deaths"] = re.compile(
                rf"(\d+)\s*(?:người\s*)?(?:{keyword_pattern})|(?:{keyword_pattern})\s*(?:khoảng\s*)?(\d+)\s*(?:người)?",
                re.IGNORECASE
            )
        elif impact_type == "missing":
            patterns["missing"] = re.compile(
                rf"(\d+)\s*(?:người\s*)?(?:{keyword_pattern})|(?:{keyword_pattern})\s*(?:khoảng\s*)?(\d+)\s*(?:người)?",
                re.IGNORECASE
            )
        elif impact_type == "injured":
            patterns["injured"] = re.compile(
                rf"(\d+)\s*(?:người\s*)?(?:{keyword_pattern})|(?:{keyword_pattern})\s*(?:khoảng\s*)?(\d+)\s*(?:người)?",
                re.IGNORECASE
            )
        elif impact_type == "damage":
            patterns["damage"] = re.compile(
                rf"(?:{keyword_pattern})\s*(?:khoảng\s*)?(?:ước\s*tính\s*)?(\d+(?:[.,]\d+)?)\s*(?:tỉ|tỷ|triệu)\s*(?:đồng)?|(\d+(?:[.,]\d+)?)\s*(?:tỉ|tỷ|triệu)\s*đồng",
                re.IGNORECASE
            )
