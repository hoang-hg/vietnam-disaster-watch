...
def stats_summary(hours: int = Query(24, ge=1, le=720), db: Session = Depends(get_db)):
    since = datetime.utcnow() - timedelta(hours=hours)
    total_articles = db.query(Article).filter(Article.published_at >= since).count()
    total_events = db.query(Event).filter(Event.last_updated_at >= since).count()

    types = ["storm", "flood", "landslide", "earthquake", "tsunami", "wind_hail", "wildfire", "extreme_weather", "unknown"]
    by_type = {t: db.query(Event).filter(Event.last_updated_at >= since, Event.disaster_type == t).count() for t in types}
