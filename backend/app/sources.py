...
VIP_TERMS = [
    ...
    r"công\s*điện\s*(?:khẩn|hỏa\s*tốc|của\s*thủ\s*tướng|của\s*phó\s*thủ\s*tướng)\s*về\s*(?:bão|lũ|thiên\s*tai|ứng\s*phó|ngập|sạt\s*lở|khắc\s*phục|tìm\s*kiếm)",
    r"chỉ\s*thị.*(?:bão|lũ|thiên\s*tai|ứng\s*phó|khẩn\s*cấp)",
    r"lệnh\s*(?:sơ\s*tán|di\s*dời)\s*(?:khẩn|khẩn\s*cấp).*(?:bão|lũ|ngập|sạt|thiên\s*tai|vỡ\s*đê|hồ\s*đập)",
    r"sơ\s*tán\s*khẩn\s*cấp.*(?:bão|lũ|sạt|ngập|lụt|cháy\s*rừng)",
    r"công\s*bố\s*tình\s*huống\s*khẩn\s*cấp.*(?:thiên\s*tai|bão|lũ|sạt\s*lở)",
    r"ban\s*bố\s*tình\s*trạng\s*khẩn\s*cấp.*(?:thiên\s*tai|bão|lũ|sạt\s*lở)",
    ...
    r"vỡ\s*(?:đê|đập|hồ|kè)(?:\s*(?:nghiêm\s*trọng|khẩn\s*cấp))?",
    r"hồ\s*chứa\s*(?:(?!\.).)*\s*(?:vỡ|xả\s*lũ\s*khẩn\s*cấp)",
    r"sự\s*cố\s*(?:đê\s*điều|hồ\s*đập|đập|hồ|kè|cống)\s*(?:nghiêm\s*trọng|khẩn\s*cấp|do\s*mưa\s*lũ)",
    ...
    r"hỗ\s*trợ\s*khẩn\s*cấp.*(?:thiên\s*tai|bão|lũ|ngập|sạt)",
    r"viện\s*trợ.*(?:thiên\s*tai|bão|lũ)",
    r"ủng\s*hộ\s*đồng\s*bào.*(?:bão|lũ|thiên\s*tai|vùng\s*sạt)",
    r"(?:tiếp\s*nhận|trao\s*tặng).*hỗ\s*trợ.*(?:bão|lũ|thiên\s*tai|sạt\s*lở)",
    r"khắc\s*phục\s*hậu\s*quả\s*(?:sau\s*)?(?:thiên\s*tai|bão|lũ|mưa\s*bão|ngập|sạt\s*lở|dông\s*lốc)",
    r"sạt\s*lở\s*(?:nghiêm\s*trọng|gây\s*tắc|chia\s*cắt|núi|đất|bờ\s*sông|bờ\s*biển)",
    r"cấm\s*(?:đường|phương\s*tiện)\s*(?:do|vì)\s*(?:sạt\s*lở|mưa\s*lũ|bão|ngập\s*sâu)",
    r"tàu\s*cá.*mất\s*liên\s*lạc.*(?:bão|gió|sóng|biển\s*động)",
    r"gặp\s*nạn\s*trên\s*biển.*(?:do|trong)\s*(?:bão|sóng|gió)",
    r"thương\s*vong\s*(?:lớn|nặng\s*nề|nghiêm\s*trọng).*(?:do|vì)\s*(?:thiên\s*tai|bão|lũ|sạt|lốc)",
    ...
    r"chết\s*người\s*(?:do|vì|trong)\s*(?:lũ|bão|ngập|sạt|vỡ|thiên\s*tai|lốc|sét)",
    ...
    r"tàu\s*.*mắc\s*cạn.*(?:do|trong)\s*(?:bão|sóng|gió)",
    r"tìm\s*kiếm\s*(?:ngư\s*dân|nạn\s*nhân|người|thi\s*thể).*mất\s*tích.*(?:do|trong|sau)\s*(?:bão|lũ|sạt|lốc|sóng)",
    r"hỗ\s*trợ.*khắc\s*phục.*(?:thiên\s*tai|bão|lũ|sạt|ngập)",
