...
DISASTER_RULES = [
  ...
  ("recovery", [
    ...
    r"thuyền\s*viên\s*mất\s*tích", r"ngư\s*dân\s*mất\s*tích", r"tàu\s*cá\s*mất\s*tích", r"trục\s*vớt\s*tàu"
  ]),
]
...
ABSOLUTE_VETO = [
    ...
    r"trào\s*lưu\s*(?:đi\s*cà\s*phê|quẩy|sống\s*ảo|check-in|hot|mới|tik\s*tok)|trend\s*(?:mới|hot)",
    ...
    r"sạt\s*lở\s*(?:niềm\s*tin|danh\s*tiếng|hình\s*ảnh|tài\s*chính|đạo\s*đức|thị\s*trường|cổ\s*phiếu)",
    ...
    r"\b(?:quán\s*bar|pub|nhạc\s*sống|phòng\s*trà|vũ\s*trường|karaoke|massage|bida)\b",
    r"\b(?:xác\s*pháo|pháo\s*hoa|bắn\s*pháo\s*hoa|lễ\s*hội\s*pháo\s*hoa|pháo\s*tết)\b",
    # SPORTS
    # SPORTS & COMPETITIONS
    ...
    r"\b(?:cháy\s*nhà\s*trọ|cháy\s*quán|cháy\s*xưởng|cháy\s*xe|hỏa\s*hoạn\s*tại\s*nhà\s*dân|cháy\s*chung\s*cư|cháy\s*căn\s*hộ|cháy\s*biệt\s*thự|cháy\s*kho|cháy\s*xe\s*bồn)(?!.*(?:bão|lũ|thiên\s*tai|sét\s*đánh|rừng|cứu\s*nạn|người\s*chết|dập\s*lửa|tử\s*vong|thiệt\s*mạng|thương\s*vong))\b",
    ...
    r"\b(?:hố\s*ga|giếng\s*hoang|hố\s*công\s*trình)(?!.*(?:ngập|lũ|bão|sạt|mưa|triều\s*cường))\b",
    ...
    r"\b(?:việc\s*nhẹ\s*lương\s*cao|lừa\s*bán|campuchia|casino|biên\s*giới\s*tây\s*nam|xuất\s*cảnh\s*trái\s*phép|người\s*di\s*cư|di\s*cư\s*bất\s*hợp\s*pháp)(?!.*(?:bão|lũ))\b",
    ...
    r"^(?:tin|thời\s*sự)\s*(?:thế\s*giới|quốc\s*tế)\b(?!.*(?:động\s*đất|sóng\s*thần|bão|lũ|thiên\s*tai|thảm\s*họa|vỡ\s*đập|cháy|sập|tai\s*nạn|cứu\s*hộ|cứu\s*nạn|cứu\s*trợ))",
    r"\b(?:liên\s*hợp\s*quốc|liên\s*minh|hội\s*đồng\s*bảo\s*an|nato|g7|g20|cop\d+)(?!.*(?:hỗ\s*trợ\s*việt\s*nam|bão|lũ|thiên\s*tai\s*tại\s*việt\s*nam|khẩn\s*cấp))\b",
    ...
    r"\b(?:rule\s*of|earning\s*app|in\s*chẵn\s*lẻ|canon\s*2900|baccarat\s*live|vivu88|88\s*clb|tặng\s*bạn|ưu\s*đãi\s*tân\s*thủ)\b",
    ...
    r"\b(?:nhồi\s*máu\s*cơ\s*tim|đột\s*quỵ|tai\s*biến|dấu\s*hiệu\s*cảnh\s*báo|triệu\s*chứng\s*bệnh|căn\s*bệnh|bác\s*sĩ\s*khuyến\s*cáo|tư\s*vấn\s*tâm\s*lý|sức\s*khỏe\s*tâm\s*thần|đuối\s*nước\s*khi\s*tắm|tắm\s*biển|tắm\s*sông|ao\s*nhà|bể\s*bơi|hồ\s*bơi|đá\s*bóng|đá\s*banh|vận\s*động\s*viên|bác\s*sĩ\s*tư\s*vấn|chăm\s*sóc\s*sức\s*khỏe|tuyệt\s*thực|bỏ\s*đói|ung\s*thư|thalassemia|vô\s*sinh)(?!.*(?:bão|lũ|ngập|sạt|thiên\s*tai|tai\s*nạn))\b",
    ...
    r"\b(?:bảo\s*hiểm\s*thất\s*nghiệp|trợ\s*cấp\s*thất\s*nghiệp|mức\s*đóng\s*bảo\s*hiểm|hưởng\s*trợ\s*cấp|trợ\s*cấp\s*xã\s*hội)\b",
    r"\b(?:lương\s*hưu|tăng\s*lương|cải\s*cách\s*tiền\s*lương|chế\s*độ\s*hưu\s*trí|tuổi\s*nghỉ\s*hưu|điều\s*chỉnh\s*lương)\b",
    r"\b(?:tuyến\s*metro|đường\s*sắt\s*đô\s*thị|tàu\s*điện\s*ngầm|đường\s*sắt\s*trên\s*cao|ga\s*ngầm)(?!.*(?:ngập|lũ|sạt\s*lở))\b",
    ...
    r"\b(?:bảo\s*hiểm\s*xã\s*hội|bhxh|hưu\s*trí|lương\s*hưu|quỹ\s*hưu\s*trí|đóng\s*bảo\s*hiểm|trợ\s*cấp\s*thất\s*nghiệp|xuất\s*khẩu\s*lao\s*động)\b",
    ...
    r"\b(?:đại\s*hội\s*cổ\s*đông|hội\s*đồng\s*quản\s*trị|hđqt|báo\s*cáo\s*tài\s*chính|cổ\s*phiếu\s*quỹ|vốn\s*hóa\s*thị\s*trường|niêm\s*yết\s*sàn|trái\s*phiếu\s*doanh\s*nghiệp|chốt\s*quyền\s*trả\s*cổ\s*tức|chi\s*trả\s*cổ\s*tức)\b",
    ...
    r"\b(?:mô\s*hình\s*sinh\s*kế|hỗ\s*trợ\s*sinh\s*kế|chuyển\s*đổi\s*sinh\s*kế|sinh\s*kế\s*bền\s*vững)(?!.*(?:bão|lũ|thiên\s*tai|khôi\s*phục|phục\s*hồi|hậu\s*quả|vùng\s*un|vùng\s*ngập))\b",
    r"\b(?:buộc\s*thực\s*hiện\s*biện\s*pháp\s*khắc\s*phục\s*hậu\s*quả|xử\s*phạt\s*vi\s*phạm\s*hành\s*chính|nghiệm\s*thu\s*công\s*trình|khai\s*thác\s*khoáng\s*sản\s*trái\s*phép)\b",
    r"\b(?:hiến\s*máu\s*nhân\s*đạo|hành\s*trình\s*đỏ|giọt\s*máu\s*nghĩa\s*tình|ngân\s*hàng\s*máu|tình\s*nguyện\s*viên\s*hiến\s*máu)\b",
    r"\b(?:khắc\s*phục\s*hậu\s*quả\s*vụ\s*cháy|khắc\s*phục\s*hậu\s*quả\s*tai\s*nạn|điều\s*tra\s*nguyên\s*nhân\s*vụ\s*tai\s*nạn|khám\s*nghiệm\s*hiện\s*trường\s*vụ\s*cháy)\b",
    ...
    r"\b(?:mời\s*bạn\s*xem\s*thêm|tin\s*liên\s*quan|bài\s*viết\s*cùng\s*chủ\s*đề|xem\s*thêm\s*vụ\s*việc|tin\s*nóng\s*trong\s*ngày)\b",
    r"\b(?:đăng\s*ký\s*tư\s*vấn|liên\s*hệ\s*quảng\s*cáo|hợp\s*tác\s*truyền\s*thông|phòng\s*kinh\s*doanh|bản\s*quyền\s*thuộc\s*về)\b",
    ...
    r"\b(?:dự\s*án\s*luật|thông\s*cáo\s*báo\s*chí|kỳ\s*họp\s*thứ|họp\s*báo\s*thường\s*kỳ|quốc\s*hội\s*khóa|đoàn\s*đại\s*biểu|hđnd\s*tỉnh|ubnd\s*tỉnh|văn\s*phòng\s*chính\s*phủ)(?!.*(?:chỉ\s*đạo|khẩn\s*cấp|công\s*điện|bão|lũ|thiên\s*tai|PCTT|MARD|cứu\s*trợ))\b",
    ...
    r"\b(?:xổ\s*số|k\s*q\s*x\s*s|vietlott|lô\s*đề|soi\s*cầu|vé\s*số|thưởng\s*độc\s*đắc|trúng\s*giải\s*đặc\s*biệt)\b",
    r"\b(?:10|5|event|sự\s*kiện)\s*nổi\s*bật(?:\s*của\s*tỉnh|\s*trong\s*năm|\s*địa\s*phương)\b",
    r"\b(?:thành\s*tựu|kết\s*quả)\s*nổi\s*bật\b",
    r"\b(?:phấn\s*đấu|mục\s*tiêu|kế\s*hoạch)\s*tăng\s*trưởng\b",
    r"\btăng\s*trưởng\s*\d+\s*con\s*số\b",
    ...
    r"\b(?:cà\s*phê|quán\s*bar|pub|lounge|vũ\s*trường|karaoke)\s*(?:khai\s*trương|giảm\s*giá|check-in|chill|quẩy|nhạc\s*sống)(?!.*(?:cháy|sập|ngập|bão|lũ))\b",
    r"\b(?:xác\s*pháo|pháo\s*giấy|pháo\s*điện|pháo\s*hoa|đốt\s*pháo)(?!.*(?:cháy|nổ|thương\s*vong|cứu\s*hộ))\b",
    r"\b(?:giếng\s*hoang|hố\s*ga|cống\s*thoát\s*nước)\s*(?:bỏ\s*hoang|không\s*nắp|nguy\s*hiểm)(?!.*(?:ngập|lũ|mưa|bão))\b",
    r"\b(?:hướng\s*dẫn\s*viên|thuyết\s*minh\s*viên)\s*(?:du\s*lịch|bảo\s*tàng|di\s*tích)(?!.*(?:mắc\s*kẹt|cô\s*lập|lũ|bão))\b",
    r"\b(?:luật\s*mới|luật\s*sửa\s*đổi|dự\s*thảo\s*luật|thông\s*qua\s*luật|hiệu\s*lực\s*thi\s*hành)(?!.*(?:phòng\s*chống\s*thiên\s*tai|đê\s*điều|khẩn\s*cấp))\b",
    ...
    r"\b(?:công\s*tác\s*xã\s*hội|quỹ\s*từ\s*thiện|vận\s*động\s*quyên\s*góp|nhà\s*hảo\s*tâm|mạnh\s*thường\s*quân|trao\s*quà\s*tình\s*nghĩa|xóa\s*đói\s*giảm\s*nghèo|lá\s*lành\s*đùm\s*lá\s*rách)(?!.*(?:bão|lũ|ngập|thiên\s*tai|khắc\s*phục|cứu\s*trợ|sạt\s*lở|đồng\s*bào|hậu\s*quả))\b",
    ...
    r"\b(?:hoa\s*đào\s*nhật\s*tân|hoa\s*mai\s*bình\s*định|lan\s*đột\s*biến|trầm\s*hương|cây\s*cảnh\s*bonsai|nghệ\s*thuật\s*tạo\s*hình\s*cây|triển\s*lãm\s*sinh\s*vật\s*cảnh)(?!.*(?:khôi\s*phục|hồi\s*sinh|lũ|bão|ngập|thiên\s*tai))\b",
    ...
    r"\b(?:phấn\s*đấu\s*đạt\s*chuẩn|nông\s*thôn\s*mới\s*nâng\s*cao|gương\s*sáng\s*tiêu\s*biểu)(?!.*(?:cứu\s*nạn|cứu\s*hộ|cứu\s*người|lũ|bão|ngập|thiên\s*tai))\b",
    ...
    r"\b(?:hiệp\s*hội\s*doanh\s*nghiệp|phòng\s*thương\s*mại|vcci|liên\s*đoàn\s*lao\s*động|hội\s*liên\s*hiệp\s*phụ\s*nữ|đoàn\s*thanh\s*niên)(?!.*(?:cứu\s*trợ|hỗ\s*trợ|khắc\s*phục|bão|lũ))\b",
    ...
    r"\b(?:triển\s*khai\s*nhiệm\s*vụ|tổng\s*kết\s*công\s*tác|phát\s*động\s*thi\s*đua|khen\s*thưởng\s*đột\s*xuất|huy\s*hiệu\s*đảng)(?!.*(?:cứu\s*nạn|cứu\s*hộ|phòng\s*chống\s*thiên\s*tai|bão|lũ))\b",
    ...
    r"\b(?:nghĩa\s*tình|nhẫn\s*cưới|gia\s*đình\s*hạnh\s*phúc|sinh\s*kế\s*phụ\s*nữ|giảm\s*nghèo\s*bền\s*vững|quà\s*tết|tiết\s*kiệm\s*tại\s*quầy)(?!.*(?:bão|lũ|thiên\s*tai|tái\s*thiết|khắc\s*phục|hỗ\s*trợ|ngập|sạt\s*lở|nhà\s*tình\s*nghĩa|nhà\s*đại\s*đoàn\s*kết))\b",
    # C) Academic & Admin
    ...
    r"\b(?:đối\s*tượng|truy\s*quét|trái\s*phép|buôn\s*lậu|bắt\s*giữ|khởi\s*tố|bắt\s*tạm\s*giam|lừa\s*đảo|chiếm\s*đoạt\s*tài\s*sản|tham\s*ô|ăn\s*chặn)(?!.*(?:cứu\s*nạn|cứu\s*hộ|cứu\s*trợ|hỗ\s*trợ|thiện\s*nguyện|khắc\s*phục|thiên\s*tai))\b",

    # E) Propaganda & Reports (Administrative praise)
    r"\b(?:phát\s*huy\s*vai\s*trò|nêu\s*gương|điển\s*hình|khen\s*thưởng|thi\s*đua|thành\s*tích|gương\s*sáng|học\s*tập\s*và\s*làm\s*theo)(?!.*(?:cứu\s*dân|cứu\s*nạn|cứu\s*người|dũng\s*cảm|quên\s*mình|hy\s*sinh|lũ\s*dữ|thiên\s*tai|bão|lũ))\b",
    r"^(?!.*(?:bão|lũ|mưa|thiên\s*tai|khắc\s*phục|cứu\s*hộ|cứu\s*nạn|vùng\s*lũ|đồng\s*bào)).*\b(?:ra\s*quân|lễ\s*phát\s*động|hưởng\s*ứng|phong\s*trào|dâng\s*hương|mít\s*tinh|kỷ\s*niệm)\b",
    ...
    # F) Market, Labor, Admin & Services - MOVED TO CONDITIONAL VETO
...
for label, pats in DISASTER_RULES:
    ...
    try:
        valid_unaccented = [v_safe(risk_lookup.strip_accents(p)) for p in pats if safe_no_accent(p)]
        if valid_unaccented:
            mega_no = re.compile("|".join(f"(?:{p})" for p in valid_unaccented), RE_FLAGS)
            compiled_no = [mega_no]
    except: pass
...
DISASTER_CONTEXT_NO_RE = []
for i, p in enumerate(DISASTER_CONTEXT):
    if safe_no_accent(p):
        DISASTER_CONTEXT_NO_RE.append((i, re.compile(v_safe(risk_lookup.strip_accents(p)), RE_FLAGS)))
...
INTERNATIONAL_LOCATIONS_NO_RE = []
for i, loc in enumerate(sources.INTERNATIONAL_LOCATIONS):
    if safe_no_accent(loc) or len(loc) >= 8:
        stripped = risk_lookup.strip_accents(loc)
        p_re = re.compile(v_safe(rf"(?<!\w){re.escape(stripped)}(?!\w)"), RE_FLAGS)
        INTERNATIONAL_LOCATIONS_NO_RE.append((i, p_re))
...
def compute_disaster_signals(text: str, title: str = "", trusted_source: bool = False, authority_level: int = 1, t_acc: str = None, t_no: str = None) -> dict:
    ...
    if INTERNATIONAL_LOCATIONS_NO_RE:
        matched_intl = {i for i, _ in enumerate(INTERNATIONAL_LOCATIONS_RE) if any(h for h in international_hits if h == sources.INTERNATIONAL_LOCATIONS[i])}
        for orig_idx, pat_re in INTERNATIONAL_LOCATIONS_NO_RE:
            if orig_idx not in matched_intl:
                if pat_re.search(t_no):
                    international_hits.append(sources.INTERNATIONAL_LOCATIONS[orig_idx])
                    matched_intl.add(orig_idx)
    ...
    score = rule_score + impact_score + agency_score + source_score + province_score + authority_bonus + context_bonus
